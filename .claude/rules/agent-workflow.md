...
- **Board scope:** GitHub Project [Model Systems & Compression](https://github.com/users/rmems/projects/6) and Linear project **Grok-1 Model Systems & Quantization** (team `rmems`) track the same work — both were renamed from "Grok Quantization", so prefer the ids over the names. Beads' Linear sync is pinned via `linear.project_id`, with `github.owner=rmems` / `github.repo=grok-ozempic`. Note the key is `github.owner`, **not** `github.org`: `bd` never reads `github.org`, so the previously documented key left `bd github status` reporting "Not configured" and the GitHub sync silently never ran (GH #96). Do not widen scope to the whole team; an unscoped sync pulled 261 issues into a ~67-issue repo. File new issues onto the project or they fall out of sync scope.
