...
No multi-GiB checkpoint is required: these cover fail-closed activation supply,
contiguous chain parsing, pack-only scale policy, and the decision rubric.
"""
...
class PackOnlyScaleTests(unittest.TestCase):
    ...
    def test_pack_v2_scales_accepted(self) -> None:
        class _FakePack:
            pack = Path("fake.goz1")
            _index = {
                "block_000.slot_00.moe_expert.gate": {
...
class DecideTests(unittest.TestCase):
    ...
    def test_fp16_failure_is_option_4(self) -> None:
        ...
        d = decide({"per_block": rows})
        self.assertEqual(d["decision"], 4)

    def test_intermediate_is_option_2(self) -> None:
        # Steady but non-runaway drift: cosine below option-1 floor, top-1 still
        # healthy, residual_in growth roughly linear (ratios < 1.8).
        chain = {
            "per_block": [
                _expert_row(0, cos=0.96, resid_in_drift=0.0),
