...
# Controlled argv execution below uses no shell; see the call-site annotations.
...
_SOURCE_BYTES = {
    str(p.relative_to(REPO_ROOT)): p.read_bytes()
...
from grok1_alpha_schedule_report import render_report  # noqa: E402
...
def validate_output_path(path):
    ...
    require(
        not any(p.startswith("grok-1-expert-precision-remedy") for p in resolved.parts),
        "output/cache collides with historical precision-remedy evidence",
    )
    require(
        resolved != REPO_ROOT and resolved != REPO_ROOT.parent,
...
def git_output(*arguments):
    ...
    # Internal callers supply literal read-only Git verbs and source paths.
    # The executable is resolved above; argv is passed without shell expansion.
...
def clean_implementation():
    ...
    require(
        not git_output("status", "--porcelain", "--untracked-files=all", "--", "scripts", "src"),
        "implementation has uncommitted sources",
    )
    for relative, resident in _SOURCE_BYTES.items():
        require(
            git_output("show", f"{commit}:{relative}")
            == resident
            == (REPO_ROOT / relative).read_bytes(),
            f"implementation source changed: {relative}",
        )
    require(
        git_output("rev-parse", "HEAD").decode("ascii").strip() == commit,
...
def tensor_inventory(path, block):
    ...
    require(
        array.ndim == 3 and array.dtype == np.dtype("float32") and all(array.shape),
    ...
    result = {
        ...
        "source_payload_bytes": int(array.nbytes),
        "source_file_bytes": Path(path).stat().st_size,
    }
    del array
    return result
...
def disk_requirements(out, side_root, inventory):
    amounts = {}
    for path, additional in (
        (out, 64 * 1024**2),
        (side_root, estimated_additional_bytes(inventory)),
    ):
...
def resource_snapshot(requirements):
    ...
    return {
        ...
        "competing": competing,
        "disk_free": {
            key: shutil.disk_usage(row["path"]).free for key, row in requirements.items()
        },
    }
...
def check_resources(snapshot, requirements):
    ...
    for key, additional in requirements.items():
        require(
            snapshot["disk_free"][key] >= 40 * GIB + additional,
...
def _group_rss(group):
    return sum(row["rss"] for row in _process_rows().values() if row["group"] == group)


def _unfinished_descendants(group):
    return any(
        pid != group and row["group"] == group and row["rss"] > 0
        for pid, row in _process_rows().items()
    )
...
def run_child(command, log, timeout=CHILD_TIMEOUT_SECONDS):
        ...
        try:
            while True:
                ...
                if proc.poll() is not None:
                    unfinished = _unfinished_descendants(proc.pid)
                    break
    ...
    return {
        ...
        "unfinished_descendants": unfinished,
        "timed_out": timed_out,
...
def _collect_cell(command, dest, cell, run_id, context):
    observation = run_child(command, dest / "child.log")
    atomic_json(dest / "execution.json", observation)
    require(
        observation["returncode"] == 0 and not observation["timed_out"],
    ...
    if context.get("provenance"):
        require(
            evidence["provenance"] == context["provenance"], "child differs from launch identities"
        )
    return evidence
...
def _nonexpert_bytes(inventory):
    nonexpert = 0
    ...
    for folder in {p.parent for p in expert_paths}:
        for path in folder.glob("*.npy"):
            if path not in expert_paths:
                ...
                del array
    return int(nonexpert)


def _quantized_tensor_cost(row, spec, side_root):
    ...
    require(
        q.dtype == np.dtype("int8") and list(q.shape) == row["shape"], "invalid actual code storage"
    )
    require(
        s.dtype == np.dtype("float32") and list(s.shape) == [row["shape"][0], row["shape"][-1]],
        "invalid actual scale storage",
    )
    return int(q.size), int(s.nbytes), qpath.stat().st_size, spath.stat().st_size


def actual_resources(inventory, cell, side_root, out):
    """Separate actual NPY storage from logical INT4 and hypothetical packing."""
    spec = CELLS[cell]
    qcount = qfiles = scales = scalefiles = fp16 = 0
    ...
    return {
        "quantized_parameters": int(qcount),
        "logical_code_bits": int(qcount * 4),
        "code_dtype": "int8",
        "actual_code_payload_bytes": int(qcount),
        "code_file_bytes": qfiles,
        "scale_payload_bytes": int(scales),
        "scale_file_bytes": scalefiles,
        "fp16_expert_payload_bytes": fp16,
        "measured_expert_payload_bytes": int(qcount + scales + fp16),
        "high_precision_nonexpert_payload_bytes": int(nonexpert),
        "measured_block_payload_bytes": int(qcount + scales + fp16 + nonexpert),
        "hypothetical_nibble_code_bytes": int((qcount + 1) // 2),
        "cache_disk_bytes": disk_bytes(side_root),
        "scratch_disk_bytes": disk_bytes(out),
