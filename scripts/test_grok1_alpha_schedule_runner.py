...
class RunnerTests(unittest.TestCase):
    def setUp(self):
        self.assertIsNotNone(importlib.util.find_spec("grok1_alpha_schedule_runner"),
    ...
    def test_resource_boundaries_and_rejection_before_launch(self):
        good = {"mem_available": 24*GIB, "swap_free": 2*GIB, "competing": [],
                "disk_free": {"out": 40*GIB+123}}
        self.r.check_resources(good, {"out": 123})
        for field, value in (("mem_available", 24*GIB-1), ("swap_free", 2*GIB-1),
                             ("disk_free", {"out": 40*GIB+122}), ("competing", [17])):
            ...
            with redirect_stdout(stdout), redirect_stderr(stderr):
                result = self.r.supervise(out, lambda: {},
                                          lambda *a: [sys.executable, "-c", code],
                                          lambda: self.r.check_resources(bad, {"out": 123}))
            self.assertNotEqual(result, 0)
            diagnostic = {"mem_available": "MemAvailable below 24 GiB", "swap_free": "free swap below 2 GiB",
                          "disk_free": "filesystem out: need", "competing": "competing workload PIDs: [17]"}[field]
    ...
    def command(self, cell, out, run_id, context):
        ...
        return [sys.executable, "-c",
                f"from pathlib import Path; Path({str(out/'metrics.json')!r}).write_text({json.dumps(data)!r})"]

    def test_all_cells_publish_literal_contrasts(self):
        ...
        self.assertEqual(data["paired_contrasts"]["chain_exit_drift"]["contrasts"]["B-A"], 0.)
    ...
    def test_report_columns_match_headers_after_cell_reordering_or_json_reload(self):
        ...
        for order, reload_json in (("DCBA", False), ("ABCD", True), ("DCBA", True)):
            ...
            for cell, drift in {"A": .1, "B": .3, "C": .4, "D": .9}.items():
                cells[cell]["chain"]["end_of_chain"]["expert_only_chain_exit"]["residual_drift_relative_norm"] = drift
            payload = analyze(cells)
            if reload_json:
                payload = json.loads(canonical_json(payload))
            report = self.r.render_report(payload)
            with self.subTest(order=order, reload_json=reload_json):
                row = next(line for line in report.splitlines() if line.startswith("| chain_exit_drift |"))
                self.assertEqual(row, "| chain_exit_drift | lower | 0.1 | 0.3 | 0.4 | 0.9 | 0.2 | 0.5 | 0.3 | 0.6 | 0.3 |")

    def test_nonfinite_raw_child_metrics_are_preserved_but_never_published_complete(self):
        for label, bad in (("nan", float("nan")), ("inf", float("inf")), ("minus-inf", float("-inf"))):
            out = self.root/label
            raw_outputs = {}
            def command(cell, dest, run_id, context):
                ...
                return [sys.executable, "-c",
                        f"from pathlib import Path; Path({str(dest/'metrics.json')!r}).write_text({raw!r})"]
            stdout, stderr = io.StringIO(), io.StringIO()
            with self.subTest(nonfinite=label), redirect_stdout(stdout), redirect_stderr(stderr):
                ...
                self.assertIn("A.chain.per_block[0].expert_only.router_margin_mean_observed", stderr.getvalue())
        ...
        for failure in ("missing", "malformed", "nonzero", "kill", "stale", "terminal"):
            ...
            with self.subTest(failure=failure):
                ...
                with redirect_stdout(stdout), redirect_stderr(stderr):
                    code = self.r.supervise(out, lambda: {}, command, lambda: None)
                self.assertNotEqual(code, 0)
                diagnostic = {"missing": "FileNotFoundError", "malformed": "JSONDecodeError",
                              "nonzero": "child failure", "kill": "child failure",
                              "stale": "stale run identity", "terminal": "end_of_chain"}[failure]
                self.assertIn(diagnostic, stderr.getvalue())
                self.assertEqual(stdout.getvalue(), "")
                data = json.loads((out/"outcome.json").read_text())
    ...
    def test_timeout_kills_process_group_and_records_resources(self):
        ...
        result = self.r.run_child([sys.executable, "-c", "import time; time.sleep(10)"], log, .15)
    ...
    def test_git_boundary_rejects_dirty_or_changed_source(self):
        ...
        for bad_command in ("status", "show", "rev-parse"):
            ...
            with self.subTest(bad_command=bad_command), patch.object(self.r, "git_output", side_effect=broken):
                with self.assertRaises(ValueError):
    ...
    def test_actual_resource_files_and_hp_retention(self):
        ...
        for b in range(4):
            ...
            np.save(side/f"block_{b:03d}"/f"{path.stem}__q_int8.npy", np.ones((2, 3, 4), dtype=np.int8))
    ...
    def test_cli_help_and_preflight_failure_without_model_or_git(self):
        ...
        stdout, stderr = io.StringIO(), io.StringIO()
        with patch.object(self.r, "git_output", side_effect=AssertionError("Git must not run")), redirect_stdout(stdout), redirect_stderr(stderr):
            code = cli.main(["--npy-root", str(self.root/"missing"), "--pack-root", str(self.root/"missing"),
                             "--embedding-shard", str(self.root/"missing.npy"), "--out", str(out),
                             "--preflight-only"])
        self.assertNotEqual(code, 0)
    ...
    def test_real_supervisor_signal_reaps_running_child(self):
        ...
        program = (f"import sys; sys.path.insert(0, {str(Path(__file__).parent.resolve())!r}); "
        ...
        proc = subprocess.Popen([sys.executable, "-c", program], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        try:
    ...
    def test_cell_entry_uses_frozen_chain_arguments_and_real_content_binding(self):
        ...
        for b in range(4):
            ...
            for i, role in enumerate(("gate", "down", "up")):
                np.save(folder/f"block_{b:03d}__slot_{i:02d}__moe_expert__{role}.npy",
                        np.ones((2, 3, 4), dtype=np.float32))
            (packs/f"block_{b:03d}-attention_plus_expert.goz1").write_bytes(b"synthetic pack identity")
        ...
        args = cli.build_parser().parse_args([
            "--npy-root", str(npy), "--pack-root", str(packs), "--embedding-shard", str(embedding),
            "--out", str(out), "--cell", "C", "--manifest", str(out/"launch.json")])
        ...
        def snapshot(requirements):
            return {"mem_available": 24*GIB, "swap_free": 2*GIB, "competing": [],
                    "disk_free": {k: 40*GIB+r["additional"] for k, r in requirements.items()}}
        with patch.object(self.r, "git_output", side_effect=git), patch.object(self.r, "resource_snapshot", side_effect=snapshot):
            context = cli.prepare_context(paths, inventory)
            (out/"launch.json").write_text(json.dumps({"parent_pid": os.getppid(), "cell": "C",
                                                     "run_id": "fresh-run", "context": context}))
            def forward(blocks, chain_paths, **kwargs):
                self.assertEqual(blocks, [0, 1, 2, 3])
                self.assertEqual((kwargs["tokens"], kwargs["seed"], kwargs["top_k"]), (8192, 20260806, 2))
